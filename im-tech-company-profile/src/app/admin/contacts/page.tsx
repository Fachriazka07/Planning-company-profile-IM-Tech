            ...
            {contacts.map((contact) => (
              <tr key={contact.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <p className="text-sm text-slate-600 line-clamp-1 max-w-xs">{contact.subject}</p>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-xs font-medium ${classes[status] || 'bg-slate-100 text-slate-600'}`}
    >
      {status.toLowerCase()}
    </span>
  );
